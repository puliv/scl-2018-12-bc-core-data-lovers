...
describe('championManage.sortData', () => {
  ...
  it('Debería devolver el champion "Zyra" para championManage.sortData(championData, "name", "descending")[0]', () => {
    window.assert.deepEqual(window.championManage.sortData(window.championsTest(), "name", "descending")[0].name, "Zyra");
  });
...
describe('championManage.filterData', () => {
  ...
  it('Debería devolver a los champions "Ahri, Akali, Ekko, Evelynn, Fiora, Fizz, Irelia, Jax, Jhin, Kassadin, Katarina, Kayn, Kha\'Zix, LeBlanc, Lee Sin, Malzahar, Master Yi, Nidalee, Nocturne, Panteon, Riven, Shaco, Talon, Teemo, Tristana, Tryndamere, Twitch, Vayne, Vi, Xerath, Xin Zhao, Yasuo, Zed" para championManage.filterData(championData, ["Assassin"])', () => {
    let champsAssa = () => {
      let namesArray = [];
      championManage.filterData(window.championsTest(), ["Assassin"]).forEach(champ => {
